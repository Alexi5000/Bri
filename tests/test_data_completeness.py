...
class TestDataCompleteness:
    ...
    def test_video_has_object_detections(self, db, processed_video_id):
        ...
        assert object_count > 0, f"Expected at least 1 object detection, got {object_count}"
    ...
    def test_no_missing_data_after_processing(self, db, video_service, processed_video_id):
        ...
        print(f"{'=' * 60}")
        print(
            f"Frames: {status['frames']['count']} ({'✓' if status['frames']['complete'] else '✗'})"
        )
        print(
            f"Captions: {status['captions']['count']} ({'✓' if status['captions']['complete'] else '✗'})"
        )
        print(
            f"Transcripts: {status['transcripts']['count']} ({'✓' if status['transcripts']['complete'] else '✗'})"
        )
        print(
            f"Objects: {status['objects']['count']} ({'✓' if status['objects']['complete'] else '✗'})"
        )
