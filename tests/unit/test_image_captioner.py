...
# The optional ``[ai]`` extra ships torch + transformers; skip the whole
...
pytest.importorskip("transformers", reason="transformers is part of the optional [ai] extra")
...
def mock_blip_model():
    ...
    with (
        patch("tools.image_captioner.BlipProcessor") as mock_processor_class,
...
class TestCaptionFramesBatch:
    ...
    def test_caption_batch_large_batch(self, image_captioner, mock_blip_model):
        ...
        with patch("os.path.exists", return_value=True), patch("PIL.Image.open") as mock_open:
            ...
            mock_blip_model["processor"].decode.side_effect = [f"caption {i}" for i in range(25)]
    ...
    def test_caption_batch_with_some_invalid_images(
        self, image_captioner, multiple_images, mock_blip_model
    ):
    ...
    def test_caption_batch_fallback_to_individual(
        self, image_captioner, multiple_images, mock_blip_model
    ):
    ...
    def test_caption_batch_fallback_with_invalid_image(
        self, image_captioner, multiple_images, mock_blip_model
    ):
...
class TestProcessBatch:
    ...
    def test_process_batch_all_valid_images(
        self, image_captioner, multiple_images, mock_blip_model
    ):
    ...
    def test_process_batch_mixed_valid_invalid(
        self, image_captioner, sample_image, mock_blip_model
    ):
...
class TestImageCaptionerEdgeCases:
    ...
    def test_batch_caption_exact_batch_size(self, image_captioner, mock_blip_model):
        ...
        try:
            ...
            # Mock captions
            mock_blip_model["processor"].decode.side_effect = [f"caption {i}" for i in range(10)]
