...
# The optional ``[ai]`` extra ships openai-whisper; skip the whole module
# when the dep is absent so CI without heavy ML deps still collects.
pytest.importorskip("whisper", reason="openai-whisper is part of the optional [ai] extra")
...
def mock_transcribe_result():
    ...
    return {
        ...
        "language": "en",
        "segments": [
            {"start": 0.0, "end": 2.5, "text": "Hello world.", "no_speech_prob": 0.1},
            {
                "start": 2.5,
                "end": 5.0,
            ...
            {"start": 5.0, "end": 7.0, "text": "Thank you for listening.", "no_speech_prob": 0.15},
        ],
    }
...
class TestTranscribeVideo:
    ...
    def test_transcribe_video_success(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
        ...
        assert (
            ...
            == "Hello world. This is a test transcription. Thank you for listening."
        )
    ...
    def test_transcribe_video_with_language(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
    ...
    def test_transcribe_video_handles_missing_confidence(
        self, audio_transcriber, sample_video_file
    ):
    ...
    def test_transcribe_video_different_languages(self, audio_transcriber, sample_video_file):
        ...
        for lang in languages:
            mock_result = {
                ...
                "language": lang,
                "segments": [
                    {"start": 0.0, "end": 2.0, "text": "Test text", "no_speech_prob": 0.1}
                ],
            }
            audio_transcriber.model.transcribe.return_value = mock_result
...
class TestTranscribeSegment:
    ...
    def test_transcribe_segment_success(
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=2.0, end_time=6.0
        )
    ...
    def test_transcribe_segment_at_beginning(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=3.0
        )
    ...
    def test_transcribe_segment_at_end(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=5.0, end_time=10.0
        )
    ...
    def test_transcribe_segment_single_word(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=1.0
        )
    ...
    def test_transcribe_segment_no_speech_detected(self, audio_transcriber, sample_video_file):
        ...
        mock_result = {
            "text": "Hello world",
            "language": "en",
            "segments": [{"start": 0.0, "end": 2.0, "text": "Hello world", "no_speech_prob": 0.1}],
        }
        audio_transcriber.model.transcribe.return_value = mock_result

        # Request segment outside of speech range
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=10.0, end_time=15.0
        )
    ...
    def test_transcribe_segment_with_language(
        self, audio_transcriber, sample_video_file, mock_transcribe_result
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=3.0, language="es"
        )
    ...
    def test_transcribe_segment_nonexistent_file(self, audio_transcriber):
        ...
        with pytest.raises(FileNotFoundError, match="Video file not found"):
            audio_transcriber.transcribe_segment(
                video_path="/nonexistent/video.mp4", start_time=0.0, end_time=5.0
            )
    ...
    def test_transcribe_segment_negative_start_time(self, audio_transcriber, sample_video_file):
        ...
        with pytest.raises(ValueError, match="Start and end times must be non-negative"):
            audio_transcriber.transcribe_segment(
                video_path=sample_video_file, start_time=-1.0, end_time=5.0
            )
    ...
    def test_transcribe_segment_start_after_end(self, audio_transcriber, sample_video_file):
        ...
        with pytest.raises(ValueError, match="Start time .* must be less than end time"):
            audio_transcriber.transcribe_segment(
                video_path=sample_video_file, start_time=5.0, end_time=2.0
            )
    ...
    def test_transcribe_segment_start_equals_end(self, audio_transcriber, sample_video_file):
        ...
        with pytest.raises(ValueError, match="Start time .* must be less than end time"):
            audio_transcriber.transcribe_segment(
                video_path=sample_video_file, start_time=3.0, end_time=3.0
            )

    def test_transcribe_segment_combines_multiple_segments(
        self, audio_transcriber, sample_video_file
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=1.5, end_time=4.5
        )
    ...
    def test_transcribe_segment_calculates_average_confidence(
        self, audio_transcriber, sample_video_file
    ):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=5.0
        )
    ...
    def test_transcribe_segment_partial_overlap(self, audio_transcriber, sample_video_file):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=2.0, end_time=4.0
        )
...
class TestAudioTranscriberEdgeCases:
    ...
    def test_transcribe_video_with_unknown_language(self, audio_transcriber, sample_video_file):
        ...
        mock_result = {
            "text": "Test",
            "language": "unknown",
            "segments": [{"start": 0.0, "end": 1.0, "text": "Test", "no_speech_prob": 0.5}],
        }
        audio_transcriber.model.transcribe.return_value = mock_result
    ...
    def test_transcribe_segment_exact_boundary_match(self, audio_transcriber, sample_video_file):
        ...
        mock_result = {
            "text": "Test",
            "language": "en",
            "segments": [{"start": 0.0, "end": 2.0, "text": "Test", "no_speech_prob": 0.1}],
        }
        audio_transcriber.model.transcribe.return_value = mock_result

        # Request exact segment boundaries
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=2.0
        )
    ...
    def test_transcribe_segment_with_gaps(self, audio_transcriber, sample_video_file):
        ...
        segment = audio_transcriber.transcribe_segment(
            video_path=sample_video_file, start_time=0.0, end_time=7.0
        )
