...
class TestAgentQuality:
    ...
    async def test_response_includes_timestamps(self, agent, processed_video_id):
        ...
        for i, query in enumerate(temporal_queries, 1):
            ...
            has_timestamps = (
                len(response.timestamps) > 0
                or re.search(r"\d+:\d+", response.message)  # MM:SS format
                or re.search(r"\d+\s*seconds?", response.message.lower())
                or "timestamp" in response.message.lower()
            )
    ...
    async def test_response_references_video_content(self, agent, processed_video_id, db):
        ...
        is_substantive = (
            len(response_text) > 50  # Reasonable length
            and not all(word in response_text for word in ["sorry", "cannot", "unable"])
            and any(
                word in response_text for word in ["see", "video", "showing", "appears", "contains"]
            )
        )
    ...
    async def test_response_quality_score(self, agent, processed_video_id):
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            if not any(
                word in response.message.lower() for word in ["error", "sorry", "cannot", "unable"]
            ):
                score += 25

            # Contains relevant keywords
            if any(
                word in response.message.lower() for word in ["video", "see", "showing", "appears"]
            ):
                score += 25
