...
def file_store(temp_data_dir):
    ...
    return FileStore(
        video_path=str(videos_dir), frame_path=str(frames_dir), cache_path=str(cache_dir)
    )
...
class TestEndToEndQueryResponse:
    ...
    async def test_simple_query_response(self, memory, mock_groq_client):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            # Send a simple query
            response = await agent.chat(message="Hello, who are you?", video_id=video_id)
...
class TestEndToEndConversationContinuity:
    ...
    async def test_follow_up_question_uses_context(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            # First query
            await agent.chat(message="What's happening in the video?", video_id=video_id)

            # Follow-up query
            await agent.chat(message="Can you tell me more about that?", video_id=video_id)
    ...
    async def test_separate_video_conversations(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            # Query video 1
            await agent.chat(message="What's in video 1?", video_id=video_id_1)

            # Query video 2
            await agent.chat(message="What's in video 2?", video_id=video_id_2)
...
class TestEndToEndErrorHandling:
    ...
    async def test_error_message_is_user_friendly(self, memory):
        ...
        with patch("services.agent.Groq") as mock_groq:
            ...
            try:
                agent = GroqAgent(groq_api_key="invalid_key", memory=memory)
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch.object(agent, "_gather_tool_context", new_callable=AsyncMock) as mock_gather:
                mock_gather.return_value = {
                    ...
                    "captions": [
                        {"timestamp": 90.0, "text": "Person walking", "frame_path": "frame1.jpg"}
                    ],
                    "transcripts": [],
                    "objects": [],
                    "errors": [],
                ...
                response = await agent.chat(message="What happens at 1:30?", video_id=video_id)
...
class TestEndToEndPerformance:
    ...
    async def test_conversation_history_limit(self, memory, database):
        ...
        for i in range(20):
            memory.add_memory_pair(
                video_id=video_id, user_message=f"Question {i}", assistant_message=f"Answer {i}"
            )
    ...
    async def test_memory_cleanup(self, memory, database):
        ...
        # Add messages
        memory.add_memory_pair(
            video_id=video_id, user_message="Test question", assistant_message="Test answer"
        )
