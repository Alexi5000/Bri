...
class TestToolFailureGracefulDegradation:
    ...
    @pytest.mark.asyncio
    async def test_single_tool_failure_continues_with_others(
        self, memory, mock_groq_client, database
    ):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            agent = GroqAgent(groq_api_key="test_key", memory=memory)
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                async def mock_post(url, **kwargs):
                    ...
                    # Other tools succeed
                        ...
                        mock_response.json.return_value = {
                            "status": "success",
                            "result": {"segments": [{"start": 10.0, "text": "Test transcript"}]},
                        }
                ...
                response = await agent.chat(
                    message="What's being said in the video?", video_id=video_id
                )
    ...
    @pytest.mark.asyncio
    async def test_all_tools_fail_returns_friendly_message(
        self, memory, mock_groq_client, database
    ):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                response = await agent.chat(message="What's in the video?", video_id=video_id)
                ...
                assert (
                    "error" not in response.message.lower() or "trouble" in response.message.lower()
                )
    ...
    async def test_tool_timeout_handled_gracefully(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                # Query should still return a response
                response = await agent.chat(
                    message="What's happening in the video?", video_id=video_id
                )
    ...
    async def test_partial_tool_results_used(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                response = await agent.chat(
                    message="What's happening and what's being said?", video_id=video_id
                )
...
class TestErrorMessageGeneration:
    ...
    def test_processing_error_messages_include_context(self):
        ...
        message = ErrorHandler.handle_processing_error("test_video_001", error, completed_steps)
...
class TestGracefulDegradationStrategy:
    ...
    def test_degradation_plan_with_partial_tools(self):
        ...
        plan = ErrorHandler.handle_graceful_degradation(requested_tools, available_tools, query)
    ...
    def test_degradation_includes_fallback_suggestions(self):
        ...
        assert (
            "describe" in plan["fallback_suggestion"].lower()
            or "visible" in plan["fallback_suggestion"].lower()
        )
    ...
    def test_degradation_message_maintains_personality(self):
        ...
        assert any(
            word in message.lower() for word in ["break", "moment", "chat", "found", "still"]
        )
    ...
    async def test_agent_recovers_from_groq_api_error(self, memory, database):
        ...
        with patch("services.agent.Groq") as mock_groq:
            ...
            is_friendly = any(
                ...
                for phrase in ["thinking", "moment", "try", "trouble", "again", "rephrase"]
            )
    ...
    async def test_agent_continues_after_tool_failure(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                response = await agent.chat(
                    message="Describe the video and tell me what's said", video_id=video_id
                )
    ...
    async def test_error_messages_stored_in_memory(self, memory, mock_groq_client, database):
        ...
        with patch("services.agent.Groq", return_value=mock_groq_client):
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                await agent.chat(message=user_message, video_id=video_id)
...
class TestErrorHandlerUtilities:
    ...
    def test_format_error_for_user_routes_correctly(self):
        ...
        message = ErrorHandler.format_error_for_user(
            tool_error, context={"tool_name": "frame_extractor"}
        )
        assert "frame" in message.lower() or "audio" in message.lower()
        ...
        message = ErrorHandler.format_error_for_user(
            upload_error, context={"upload": True, "filename": "test.avi"}
        )
        assert "format" in message.lower() or "mp4" in message.lower()
    ...
    def test_tool_fallback_suggestions_are_relevant(self):
        ...
        suggestion = ErrorHandler._suggest_tool_fallback("caption_frames", ["transcribe_audio"])
        assert suggestion
        assert "said" in suggestion.lower() or "audio" in suggestion.lower()

        # Transcribe fails, caption available
        suggestion = ErrorHandler._suggest_tool_fallback("transcribe_audio", ["caption_frames"])
        assert suggestion
        assert "visible" in suggestion.lower() or "see" in suggestion.lower()

        # No alternatives available
        suggestion = ErrorHandler._suggest_tool_fallback("caption_frames", [])
