...
class TestPerformanceBenchmark:
    ...
    def test_stage1_frame_extraction_performance(self, db, test_video_path):
        ...
        assert elapsed < 10 or frame_count > 0, f"Stage 1 took {elapsed:.2f}s (target: < 10s)"
    ...
    def test_stage2_caption_generation_performance(self, db, test_video_path):
        ...
        assert elapsed < 60 or caption_count > 0, f"Stage 2 took {elapsed:.2f}s (target: < 60s)"
    ...
    def test_query_response_time(self, db):
        ...
        queries = [
            (
                "Get all frames",
                "SELECT * FROM video_context WHERE video_id = ? AND context_type = 'frame'",
            ),
            (
                "Get all captions",
            ...
            (
                "Get all transcripts",
                "SELECT * FROM video_context WHERE video_id = ? AND context_type = 'transcript'",
            ),
            (
                "Get all objects",
                "SELECT * FROM video_context WHERE video_id = ? AND context_type = 'object'",
            ),
