...
class MultiTierCache:
    ...
    def __init__(self, l1_capacity: int = 100, l2_enabled: bool = None, l3_enabled: bool = True):
        ...
        logger.info(
            f"Multi-tier cache initialized: L1={True}, L2={self.l2_enabled}, L3={self.l3_enabled}"
        )
    ...
    def invalidate_pattern(self, pattern: str, namespace: str = "default") -> int:
        ...
        with self.l1_cache.lock:
            keys_to_delete = [
                k for k in self.l1_cache.cache.keys() if self._matches_pattern(k, full_pattern)
            ]
        ...
        if self.l3_enabled:
            with self.l3_lock:
                keys_to_delete = [
                    k for k in self.l3_cache.keys() if self._matches_pattern(k, full_pattern)
                ]
                for key in keys_to_delete:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if self.l3_enabled:
            with self.l3_lock:
                ...
                expired_keys = [k for k, ttl in self.l3_cache_ttl.items() if current_time > ttl]
...
def get_multi_tier_cache() -> MultiTierCache:
    ...
    if _multi_tier_cache is None:
        _multi_tier_cache = MultiTierCache(
            l1_capacity=100, l2_enabled=Config.REDIS_ENABLED, l3_enabled=True
        )
