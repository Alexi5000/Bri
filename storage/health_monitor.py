    ...
    def monitor_query(self, query: str, parameters: tuple | None = None):
        ...
        try:
            ...
            if execution_time_ms > self.slow_query_threshold_ms:
                stats["slow_count"] += 1
                logger.warning(f"Slow query detected ({execution_time_ms:.2f}ms): {query_key}")
...
class DatabaseHealthMonitor:
    ...
    def get_database_size(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"size_bytes": size_bytes, "size_mb": round(size_mb, 2), "path": str(db_path)}
    ...
    def get_growth_rate(self, days: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            context_count = self.db.execute_query("SELECT COUNT(*) as count FROM video_context")[0][
                "count"
            ]
            ...
            if video_count > 0:
                growth_rate_mb_per_day = (current_size["size_mb"] / video_count) * (
                    recent_videos / days
                )
            else:
            ...
            return {
                ...
                "estimated_size_30_days_mb": round(
                    current_size["size_mb"] + (growth_rate_mb_per_day * 30), 2
                ),
            }
    ...
    def check_index_usage(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            index_info = []
            for idx in indexes:
                index_info.append({"name": idx[0], "table": idx[1], "sql": idx[2]})
