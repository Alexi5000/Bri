...
class JSONCompressor:
    ...
    def compress(self, data: dict[str, Any]) -> bytes:
        ...
        logger.debug(
            f"JSON compressed: {original_size} → {compressed_size} bytes ({ratio:.1f}% reduction)"
        )
...
class ImageCompressor:
    ...
    def __init__(self, quality: int = 85, lossless: bool = False):
        ...
        logger.info(f"Image compressor initialized (quality: {quality}, lossless: {lossless})")
    ...
    def compress_to_webp(
        ...
        try:
            ...
            # Save as WebP
            img.save(output_path, "WEBP", quality=self.quality, lossless=self.lossless)
    ...
    def compress_to_bytes(self, input_path: str) -> bytes:
        ...
        try:
            ...
            buffer = io.BytesIO()
            img.save(buffer, "WEBP", quality=self.quality, lossless=self.lossless)
    ...
    def batch_compress(
        self, input_paths: list[str], output_dir: str | None = None
...
class FrameDeduplicator:
    ...
    def __init__(self, similarity_threshold: float = 0.95):
        ...
        logger.info(f"Frame deduplicator initialized (threshold: {similarity_threshold})")
    ...
    def is_duplicate(
        self, video_id: str, timestamp: float, image_path: str
    ...
    def clear_video(self, video_id: str) -> None:
        ...
        Args:
            video_id: Video identifier
        """
        keys_to_remove = [key for key in self.frame_hashes.keys() if key.startswith(f"{video_id}:")]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_frames_per_video": (
                sum(video_counts.values()) / len(video_counts) if video_counts else 0
            ),
        }
...
class CompressionManager:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        return {"frame_deduplication": self.frame_deduplicator.get_stats()}
