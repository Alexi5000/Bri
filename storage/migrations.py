...
class MigrationManager:
    ...
    def get_applied_migrations(self) -> list[dict[str, Any]]:
        ...
        """
        try:
            results = self.db.execute_query("SELECT * FROM schema_version ORDER BY version")
    ...
    def rollback(self, target_version: int | None = None, steps: int = 1) -> None:
        ...
        if target_version is not None:
            ...
            migrations_to_rollback = [
                m for m in reversed(self.migrations) if m.version <= current_version
            ][:steps]
...
def create_migration(
    version: int, description: str, up_sql: str, down_sql: str | None = None
) -> Migration:
    ...
    return Migration(version=version, description=description, up=up, down=down_func)
