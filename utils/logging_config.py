...
# Context variables for request-scoped logging. The default is ``None``
# and consumers must call ``_log_context.set({})`` before first use; that
# keeps the default immutable across contexts (B039).
...
class PerformanceLogger:
    ...
    def log_execution_time(
        self, operation: str, execution_time: float, success: bool = True, **kwargs
    ) -> None:
    ...
    def log_cache_hit(self, cache_key: str, hit: bool) -> None:
        ...
        extra_fields = {"cache_key": cache_key, "cache_status": status}

        log_record = self.logger.makeRecord(
            self.logger.name, logging.DEBUG, "(cache)", 0, f"Cache {status}: {cache_key}", (), None
        )
        log_record.extra_fields = extra_fields
        self.logger.handle(log_record)
...
class APILogger:
    ...
    def log_api_call(
        ...
        log_record = self.logger.makeRecord(self.logger.name, level, "(api)", 0, message, (), None)
        log_record.extra_fields = extra_fields
        self.logger.handle(log_record)
...
def setup_logging(
    ...
    root_logger.info(
        f"Logging initialized: level={log_level}, dir={log_dir}, rotation={enable_rotation}"
    )
...
class LogContext:
    ...
    def __enter__(self):
        ...
        current_context = {} if previous is None else previous.copy()
...
class AuditLogger:
    ...
    def log_data_mutation(
        ...
        **kwargs,
        ...
        log_record = self.logger.makeRecord(
            self.logger.name, logging.INFO, "(audit)", 0, message, (), None
        )
        log_record.extra_fields = extra_fields
        self.logger.handle(log_record)
...
class PipelineLogger:
    ...
    def log_stage(
        ...
        **kwargs,
        ...
        log_record = self.logger.makeRecord(
            self.logger.name, level, "(pipeline)", 0, message, (), None
        )
        log_record.extra_fields = extra_fields
        self.logger.handle(log_record)
