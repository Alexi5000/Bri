...
class ErrorHandler:
    ...
    def handle_tool_error(
        tool_name: str, error: Exception, available_tools: list[str] | None = None
    ) -> str:
        """
        ...
        base_message = ErrorHandler.TOOL_ERROR_MESSAGES.get(
            tool_name, f"I had trouble with {tool_name}, but I'll do my best with what I have!"
        )

        # Add fallback suggestions if other tools are available
        if available_tools:
            fallback_msg = ErrorHandler._suggest_tool_fallback(tool_name, available_tools)
            if fallback_msg:
    ...
    def suggest_fallback(
        original_query: str, available_data: list[str], failed_tools: list[str] | None = None
    ) -> str:
        """
    ...
    def handle_graceful_degradation(
        requested_tools: list[str], available_tools: list[str], query: str
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "fallback_suggestion": ErrorHandler.suggest_fallback(
                query, usable_tools, unavailable_tools
            )
            if usable_tools
            else None,
        }
    ...
    def handle_video_upload_error(error: Exception, filename: str) -> str:
        ...
        if "format" in error_str or "codec" in error_str:
            ...
            return (
                "I don't have permission to access that file. Could you check the file permissions?"
            )
    ...
    def handle_processing_error(
        video_id: str, error: Exception, completed_steps: list[str] | None = None
    ) -> str:
        """
    ...
    @staticmethod
    def _suggest_tool_fallback(failed_tool: str, available_tools: list[str]) -> str | None:
        """
    ...
    @staticmethod
    def format_error_for_user(error: Exception, context: dict[str, Any] | None = None) -> str:
        """
        ...
        if context.get("tool_name"):
            return ErrorHandler.handle_tool_error(
                context["tool_name"], error, context.get("available_tools")
            )
            ...
            return ErrorHandler.handle_video_upload_error(error, context.get("filename", "video"))

        elif context.get("processing"):
            return ErrorHandler.handle_processing_error(
                context.get("video_id", ""), error, context.get("completed_steps")
            )

        elif context.get("query"):
            return ErrorHandler.handle_query_error(context.get("query", ""), error)
