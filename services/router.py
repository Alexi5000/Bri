...
class ToolRouter:
    ...
    def analyze_query(self, query: str, context: dict[str, Any] | None = None) -> ToolPlan:
        """
        ...
        execution_order = self._optimize_execution_order(tools_needed, parameters)

        return ToolPlan(
            tools_needed=tools_needed, execution_order=execution_order, parameters=parameters
        )
    ...
    def _classify_query_type(self, query_lower: str) -> str:
        ...
        if any(
            kw in query_lower
            for kw in ["summary", "summarize", "overview", "about", "what is", "tell me"]
        ):
            return "general"
    ...
    def requires_objects(self, query: str) -> bool:
        ...
        if any(
            phrase in query_lower
            for phrase in ["how many", "count", "find all", "show all", "every time", "all the"]
        ):
            return True
