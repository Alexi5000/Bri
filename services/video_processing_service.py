...
class VideoProcessingService:
    ...
    def store_tool_results(
        ...
        if idempotency_key and self._check_idempotency(video_id, tool_name, idempotency_key):
            logger.info(
                f"Operation already completed (idempotency key: {idempotency_key}), skipping"
            )
    ...
    def _store_frames(self, video_id: str, results: dict[str, Any]) -> dict[str, int]:
        ...
        stored_count = self._batch_insert_with_retry(
            video_id, "frame", insert_data, with_lineage=True
        )

        # Record lineage
        try:
            self.lineage_tracker.record_batch_processing(video_id, context_ids, "extract_frames")
    ...
    def _store_captions(self, video_id: str, results: dict[str, Any]) -> dict[str, int]:
        ...
        # Execute batch insert with transaction
        stored_count = self._batch_insert_with_retry(
            video_id, "caption", insert_data, with_lineage=True
        )

        # Record lineage
        try:
            self.lineage_tracker.record_batch_processing(video_id, context_ids, "caption_frames")
        except Exception as e:
            logger.warning(f"Failed to record lineage: {e}")
    ...
    def _store_transcript(self, video_id: str, results: dict[str, Any]) -> dict[str, int]:
        ...
        # Execute batch insert with transaction
        stored_count = self._batch_insert_with_retry(
            video_id, "transcript", insert_data, with_lineage=True
        )

        # Record lineage
        try:
            self.lineage_tracker.record_batch_processing(video_id, context_ids, "transcribe_audio")
        except Exception as e:
            logger.warning(f"Failed to record lineage: {e}")
    ...
    def _store_objects(self, video_id: str, results: dict[str, Any]) -> dict[str, int]:
        ...
        # Execute batch insert with transaction
        stored_count = self._batch_insert_with_retry(
            video_id, "object", insert_data, with_lineage=True
        )

        # Record lineage
        try:
            self.lineage_tracker.record_batch_processing(video_id, context_ids, "detect_objects")
        except Exception as e:
            logger.warning(f"Failed to record lineage: {e}")
    ...
    def _batch_insert_with_retry(
        self, video_id: str, context_type: str, insert_data: list[tuple], with_lineage: bool = False
    ) -> int:
        """
    ...
    def _record_idempotency(self, video_id: str, tool_name: str, idempotency_key: str) -> None:
        ...
        try:
            context_id = str(uuid.uuid4())
            data = json.dumps(
                {"tool_name": tool_name, "key": idempotency_key, "completed_at": time.time()}
            )
    ...
    def delete_video_data(self, video_id: str) -> dict[str, int]:
        ...
        try:
            ...
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM video_context WHERE video_id = ?", (video_id,))
                total_count = cursor.fetchone()[0]

                # Delete all context data
                cursor.execute("DELETE FROM video_context WHERE video_id = ?", (video_id,))
