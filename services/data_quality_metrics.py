...
class DataQualityMetrics:
    ...
    def calculate_completeness(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            frames_completeness = (
                min(1.0, frame_count / expected_frames) if expected_frames > 0 else 0.0
            )
            captions_completeness = (
                min(1.0, caption_count / frame_count) if frame_count > 0 else 0.0
            )
            ...
            return {"video_id": video_id, "overall_completeness": 0.0, "error": str(e)}
    ...
    def calculate_freshness(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            last_update_str = (
                context_rows[0]["last_update"]
                if context_rows and context_rows[0]["last_update"]
                else upload_time_str
            )
    ...
    def calculate_accuracy(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            caption_accuracy = (
                sum(caption_confidences) / len(caption_confidences) if caption_confidences else 0.0
            )

            # Get transcript confidence scores
            transcript_confidences = self._get_confidence_scores(
                video_id, "transcript", "confidence"
            )
            transcript_accuracy = (
            ...
            object_accuracy = (
                sum(object_confidences) / len(object_confidences) if object_confidences else 0.0
            )
            ...
            result = {
                ...
                "transcript_accuracy": round(transcript_accuracy, 3)
                if transcript_confidences
                else None,
            ...
            return {"video_id": video_id, "overall_accuracy": 0.0, "error": str(e)}
    ...
    def calculate_volume_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            estimated_storage_mb = (
                total_frames * 100 + total_captions * 1 + total_transcripts * 5 + total_objects * 2
            ) / 1024

            growth_rate_data_mb_per_day = (
                (estimated_storage_mb / total_videos) * growth_rate_videos_per_day
                if total_videos > 0
                else 0.0
            )
        ...
        try:
            ...
            if not completeness.get("is_complete", False):
                ...
                recommendations.append(
                    f"Process missing data: {', '.join(completeness.get('missing_data', []))}"
                )
            ...
            if has_degradation:
                logger.warning(
                    f"Quality degradation detected for video {video_id}: {len(alerts)} alerts"
                )
            ...
            return {"video_id": video_id, "has_degradation": False, "error": str(e)}
    ...
    def get_system_quality_report(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Aggregate metrics
            if video_reports:
                avg_completeness = sum(r["completeness"] for r in video_reports) / len(
                    video_reports
                )
            ...
            logger.info(
                f"System quality: {report['system_health']} ({complete_percentage:.1%} complete)"
            )
            return report
