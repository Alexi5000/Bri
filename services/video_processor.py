...
class VideoProcessor:
    ...
    async def process_video(
        self, video_id: str, progress_callback: Callable[[str, float, str], None] | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if progress_callback:
                progress_callback("Starting", 0, "🚀 Getting ready to analyze your video...")
            ...
            if result.get("status") == "complete":
                ...
                if progress_callback:
                    progress_callback(
                        "Complete", 100, "✨ All set! I'm ready to answer your questions!"
                    )
                ...
                update_video_status(video_id, "complete")
                logger.warning(
                    f"Video processing partially completed for {video_id}: {result.get('errors')}"
                )
                ...
                raise VideoProcessingError(f"Processing failed with status: {result.get('status')}")

        except requests.exceptions.Timeout:
            update_video_status(video_id, "error")
            logger.error(f"Video processing timed out for {video_id}")
            raise VideoProcessingError("Processing took too long. Please try with a shorter video.")
