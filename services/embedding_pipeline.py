...
class EmbeddingPipeline:
    ...
    def __init__(self, metadata_file: str = "data/vector_db/embedding_metadata.json"):
            ...
            if transcript_segments is not None:
                if transcript_segments:
                    ...
                    if success:
                        ...
                        logger.info(
                            f"Indexed {transcript_count} transcript segments for video {video_id}"
                        )
    ...
    def update_model_version(self, new_version: str) -> None:
        ...
        self._save_metadata()
        logger.info(
            f"Updated model version from {old_version} to {new_version}. All videos marked for reindexing."
        )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        needs_reindex = sum(
            1 for v in self.metadata["videos"].values() if v.get("needs_reindex", False)
        )
        total_captions = sum(v.get("caption_count", 0) for v in self.metadata["videos"].values())
        total_transcripts = sum(
            v.get("transcript_count", 0) for v in self.metadata["videos"].values()
        )
        ...
        return {
            ...
            "semantic_search_stats": self.semantic_search.get_stats()
    ...
    def monitor_quality(self, test_queries: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            for test in test_queries:
                ...
                # Perform search
                results = self.semantic_search.search(query=query, video_id=video_id, top_k=5)
