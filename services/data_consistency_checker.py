...
class DataConsistencyChecker:
    ...
    def _check_timestamp_ordering(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if len(frame_timestamps) > 1:
                for i in range(1, len(frame_timestamps)):
                    if frame_timestamps[i] < frame_timestamps[i - 1]:
                        issues.append(
                            f"Frame timestamps out of order at index {i}: {frame_timestamps[i]} < {frame_timestamps[i - 1]}"
                        )
            ...
            return {"passed": True, "message": "All timestamps are in correct order"}
    ...
    def _check_data_corruption(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"passed": True, "message": "No data corruption detected"}
    ...
    def _check_transcript_segments(self, video_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not rows:
                return {"passed": True, "message": "No transcript segments to check"}
    ...
    def fix_timestamp_ordering(self, video_id: str, context_type: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"fixed": True, "message": "Timestamps are already in correct order"}

        except Exception as e:
            logger.error(f"Failed to fix timestamp ordering: {e}")
            return {"fixed": False, "message": f"Fix failed: {e}"}
