...
class SemanticSearchService:
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            self.collection = self.client.create_collection(
                name=self.collection_name, metadata={"description": "BRI video caption embeddings"}
            )
    ...
    def index_captions(
        self, video_id: str, captions: list[dict[str, Any]], batch_size: int = 32
    ) -> bool:
        ...
        try:
            ...
            for i in range(0, len(captions), batch_size):
                ...
                logger.debug(
                    f"Indexed batch {i // batch_size + 1}/{(len(captions) - 1) // batch_size + 1}"
                )
    ...
    def index_transcripts(
        self, video_id: str, segments: list[dict[str, Any]], batch_size: int = 32
    ) -> bool:
        ...
        try:
            ...
            for i in range(0, len(segments), batch_size):
                ...
                logger.debug(
                    f"Indexed batch {i // batch_size + 1}/{(len(segments) - 1) // batch_size + 1}"
                )

            logger.info(
                f"Successfully indexed {len(segments)} transcript segments for video {video_id}"
            )
            return True
    ...
    def hybrid_search(
        ...
        try:
            ...
            for kw_result in keyword_results:
                ...
                # Combine scores
                combined_score = (1 - semantic_weight) * kw_score_norm + semantic_weight * sem_score
