...
class FrameExtractor:
    ...
    def extract_frames(
        ...
        try:
            ...
            if duration / interval_seconds > max_frames:
                ...
                logger.info(
                    f"Using adaptive interval: {interval_seconds:.2f}s for {duration:.2f}s video"
                )
            ...
            while cap.isOpened() and extracted_count < max_frames:
                ...
                if extracted_count == 0 or current_timestamp >= extracted_count * interval_seconds:
                    ...
                    logger.debug(
                        f"Extracted frame {extracted_count}/{max_frames} at {current_timestamp:.2f}s"
                    )
    ...
    def extract_frame_at_timestamp(self, video_path: str, video_id: str, timestamp: float) -> Frame:
        """
        ...
        try:
            ...
            # Create Frame object
            frame = Frame(
                timestamp=timestamp, image_path=str(frame_path), frame_number=target_frame_number
            )
