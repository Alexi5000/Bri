...
class ObjectDetector:
    ...
    def detect_objects_in_frames(
        self, frame_paths: list[str], timestamps: list[float], confidence_threshold: float = 0.25
        ...
        for path, ts in zip(frame_paths, timestamps):
            if os.path.exists(path):
                ...
                detection_results.append(DetectionResult(frame_timestamp=ts, objects=[]))
        ...
        try:
                ...
                if result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for i in range(len(boxes)):
                        ...
                        detected_objects.append(
                            DetectedObject(
                                class_name=class_name, confidence=confidence, bbox=(x, y, w, h)
                            )
                        )

                detection_results.append(
                    DetectionResult(frame_timestamp=timestamp, objects=detected_objects)
                )
            ...
            return [DetectionResult(frame_timestamp=ts, objects=[]) for ts in timestamps]
    ...
    def search_for_object(
        ...
        all_detections = self.detect_objects_in_frames(
            frame_paths, timestamps, confidence_threshold
        )
        ...
        for detection in all_detections:
            ...
            matching_objects = [
                obj for obj in detection.objects if obj.class_name.lower() == object_class_lower
            ]
            ...
            if matching_objects:
                filtered_results.append(
                    DetectionResult(
                        frame_timestamp=detection.frame_timestamp, objects=matching_objects
                    )
                )

        logger.info(
            f"Found '{object_class}' in {len(filtered_results)} out of {len(frame_paths)} frames"
        )
    ...
    def detect_single_frame(
        self, frame_path: str, timestamp: float, confidence_threshold: float = 0.25
    ) -> DetectionResult:
        """
        ...
        """
        results = self.detect_objects_in_frames([frame_path], [timestamp], confidence_threshold)
        return results[0] if results else DetectionResult(frame_timestamp=timestamp, objects=[])
