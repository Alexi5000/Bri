...
class AudioTranscriber:
    ...
    def transcribe_video(self, video_path: str, language: str | None = None) -> Transcript:
        """
        ...
        try:
            ...
            transcript = Transcript(
                segments=segments, language=detected_language, full_text=full_text
            )

            logger.info(
                f"Transcription complete: {len(segments)} segments, language: {detected_language}"
            )
    ...
    def transcribe_segment(
        self, video_path: str, start_time: float, end_time: float, language: str | None = None
    ) -> TranscriptSegment:
        """
        ...
        try:
            logger.info(
                f"Transcribing segment: {video_path} from {start_time:.2f}s to {end_time:.2f}s"
            )

            # Transcribe the full video (Whisper doesn't support direct segment extraction)
            # Then filter to the requested time range
            result = self.model.transcribe(
                video_path, language=language, verbose=False, word_timestamps=False
            )
            ...
            if matching_segments:
                combined_text = " ".join(seg["text"].strip() for seg in matching_segments)
            ...
            logger.info(
                f"Segment transcription complete: {len(transcript_segment.text)} characters"
            )
            ...
            logger.error(
                f"Failed to transcribe segment {start_time}-{end_time} from {video_path}: {str(e)}"
            )
