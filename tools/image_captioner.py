...
class ImageCaptioner:
    ...
    def caption_frame(self, image_path: str, timestamp: float = 0.0) -> Caption:
        ...
        try:
            ...
            return Caption(frame_timestamp=timestamp, text=caption_text, confidence=confidence)

        except Exception as e:
            raise Exception(f"Failed to caption image {image_path}: {str(e)}")

    def caption_frames_batch(
        self, image_paths: list[str], timestamps: list[float]
    ) -> list[Caption]:
        """
            ...
            try:
                ...
                for path, ts in zip(batch_paths, batch_timestamps):
                    try:
                        ...
                        captions.append(
                            Caption(
                                frame_timestamp=ts, text="[Caption unavailable]", confidence=0.0
                            )
                        )

        return captions

    def _process_batch(self, image_paths: list[str], timestamps: list[float]) -> list[Caption]:
        """
