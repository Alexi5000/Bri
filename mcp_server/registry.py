...
class FrameExtractionTool(Tool):
        ...
        try:
            ...
            if "timestamp" in parameters and parameters["timestamp"] is not None:
                ...
                frame = self._get_extractor().extract_frame_at_timestamp(
                    video_path, video_id, parameters["timestamp"]
                )
                return {"frames": [frame.dict()], "count": 1}
            else:
                ...
                frames = self._get_extractor().extract_frames(
                    video_path, video_id, interval_seconds=interval, max_frames=max_frames
                )

                return {"frames": [frame.dict() for frame in frames], "count": len(frames)}
    ...
    def _get_video_path(self, video_id: str) -> str:
        ...
        db = get_database()
        result = db.execute_query("SELECT file_path FROM videos WHERE video_id = ?", (video_id,))
        if not result:
...
class ImageCaptioningTool(Tool):
    ...
    async def execute(self, video_id: str, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"captions": [caption.dict() for caption in captions], "count": len(captions)}
        except Exception as e:
        ...
        try:
            ...
            if "start_time" in parameters and "end_time" in parameters:
                ...
                segment = self._get_transcriber().transcribe_segment(
                    video_path, parameters["start_time"], parameters["end_time"], language
                )
                return {"segment": segment.dict(), "type": "segment"}
            else:
                # Transcribe full video
                transcript = self._get_transcriber().transcribe_audio(video_path, language)
                return {"transcript": transcript.dict(), "type": "full"}
        except Exception as e:
        ...
        try:
            ...
            if object_class:
                ...
                detections = self._get_detector().search_for_object(
                    frame_paths, timestamps, object_class, confidence_threshold
                )
            else:
                # Detect all objects
