...
def get_status_emoji(status: str) -> str:
    ...
    Returns:
        Emoji string
    """
    status_emojis = {"pending": "⏳", "processing": "⚙️", "complete": "✅", "error": "❌"}
...
def delete_video(video_id: str) -> None:
    ...
    try:
        ...
        st.session_state.uploaded_videos = [
            v for v in st.session_state.uploaded_videos if v["video_id"] != video_id
        ]
...
def render_video_library() -> None:
    ...
    ready_videos = sum(
        1 for v in st.session_state.uploaded_videos if v.get("processing_status") == "complete"
    )
    processing_videos = sum(
        1 for v in st.session_state.uploaded_videos if v.get("processing_status") == "processing"
    )
    ...
    with filter_col1:
        search_query = st.text_input(
            "🔍 Search videos", placeholder="Search by filename...", label_visibility="collapsed"
        )
    ...
    if search_query:
        filtered_videos = [
            v for v in filtered_videos if search_query.lower() in v["filename"].lower()
        ]
    ...
    if status_filter != "All":
        ...
        filtered_videos = [
            v for v in filtered_videos if v.get("processing_status") == status_map[status_filter]
        ]
    ...
    if filtered_videos:
        ...
        filtered_videos.sort(key=lambda v: v.get("upload_timestamp", ""), reverse=True)
