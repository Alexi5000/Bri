...
def render_conversation_history_panel(
    ...
    try:
        ...
        conversation_history = memory.get_conversation_history(
            video_id, limit=messages_per_page, offset=offset
        )
        ...
        if total_pages > 1:
            ...
            with col1:
                if current_page > 0:
                    if st.button(
                        "⬅️ Newer", key=f"history_prev_{video_id}", use_container_width=True
                    ):
            ...
            with col2:
                if current_page < total_pages - 1:
                    if st.button(
                        "Older ➡️", key=f"history_next_{video_id}", use_container_width=True
                    ):
                        st.session_state[page_key] += 1
                        st.rerun()
...
def _render_memory_wipe_button(video_id: str, memory: Memory) -> None:
    ...
    if not st.session_state[confirm_key]:
        ...
        with col1:
            if st.button(
                "✅ Yes, clear it", key=f"confirm_yes_{video_id}", use_container_width=True
            ):
                try:
        ...
        with col2:
            if st.button("❌ Cancel", key=f"confirm_no_{video_id}", use_container_width=True):
                st.session_state[confirm_key] = False
                st.rerun()
...
def get_conversation_summary(session: list[MemoryRecord]) -> str:
    ...
    return (
        f"{len(user_messages)} question{'s' if len(user_messages) != 1 else ''}: {first_question}"
    )


def load_conversation_context(session: list[MemoryRecord], video_id: str) -> None:
