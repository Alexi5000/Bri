...
class LogDashboard:
    ...
    def render(self):
        ...
        with st.sidebar:
            ...
            # Log file selection
            selected_log = st.selectbox("Log File", options=list(self.log_files.keys()), index=0)
            ...
            component_filter = st.text_input(
                "Component", placeholder="services.agent, tools.*, etc."
            )
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📜 Log Viewer", "📈 Analytics", "⚡ Performance", "🔍 Error Tracking"]
        )

        with tab1:
            self._render_log_viewer(
                selected_log, log_levels, time_range, search_query, component_filter
            )
    ...
    def _render_log_viewer(
        ...
        filtered_logs = self._filter_logs(logs, log_levels, search_query, component_filter)
    ...
    def _render_analytics(self, log_type: str, time_range: str):
        ...
        if timestamps:
            ...
            if hour_counts:
                ...
                st.line_chart(
                    data={"Time": [h.strftime("%H:%M") for h in hours], "Count": counts},
                    x="Time",
                    y="Count",
                )
        ...
        for log in logs:
            ...
            if match:
                ...
                if normalized not in error_patterns:
                    error_patterns[normalized] = {"count": 0, "examples": [], "timestamps": []}
                ...
                for line in lines:
                    ...
                    if match:
                        try:
                            ...
                            # Date parse failed; treat the line as in-range so
                            # operators still see log entries with non-standard
                            # timestamp formats.
                            filtered_lines.append(line.rstrip())
                    else:
    ...
    def _filter_logs(
        self, logs: list[str], log_levels: list[str], search_query: str, component_filter: str
    ) -> list[str]:
