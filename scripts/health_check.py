...
def show_report():
    ...
    try:
        ...
        print(
            f"   Recent Videos ({growth.get('days_analyzed', 0)} days): {growth.get('recent_videos', 0)}"
        )
        ...
        print(
            f"   Foreign Keys: {'✓ Enabled' if conn.get('foreign_keys_enabled') else '✗ Disabled'}"
        )
        ...
        print("\n⚙️  Constraints:")
        print(
            f"   Foreign Keys: {'✓ Enabled' if constraints['foreign_keys_enabled'] else '✗ Disabled'}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Database health check tool for BRI video agent")
    ...
    cleanup_parser.add_argument(
        "--keep-days", type=int, default=30, help="Number of days to keep backups (default: 30)"
    )
