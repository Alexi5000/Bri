    ...
    try:
        ...
        for table in tables:
            ...
            for col in columns:
                ...
                schema_doc.append(
                    f"| {name} | {col_type or 'ANY'} | {bool(not_null)} | {dflt_value or ''} | {bool(pk)} |"
                )
        ...
        check_constraints = {
            ...
            "memory": ["role IN (user, assistant)", 'content != ""', 'message_id != ""'],
            "video_context": [
        ...
        for table, constraints in check_constraints.items():
            log_result(
                "1.2",
                f"CHECK constraints defined for {table}",
                "PASS",
        ...
        try:
            ...
            cursor.execute("COMMIT")
            log_result(
                "1.2", "Failed transaction (rollback)", "FAIL", "Expected rollback didn't occur"
            )
            ...
            if cursor.fetchone()[0] == 0:
                log_result(
                    "1.2", "Failed transaction (rollback)", "PASS", "Rollback worked correctly"
                )
            else:
                log_result(
                    "1.2",
                    ...
                    "FAIL",
                    "Rollback failed, data persisted",
                )
        ...
        try:
            ...
            cursor.execute("ROLLBACK")
            log_result(
                "1.2", "Foreign key constraint enforcement", "PASS", "FK violation prevented"
            )
    ...
    for model_dir in model_dirs:
        if model_dir.exists():
            ...
            log_result(
                ...
                "WARN",
                "Directory not found (will download on first use)",
            )
...
def task_1_10_gap_analysis():
    ...
    for task, tests in VERIFICATION_RESULTS.items():
        ...
        print(f"\n{task}:")
        print(
            f"  Total: {len(tests)} | ✓ Pass: {task_passes} | ✗ Fail: {task_fails} | ⚠ Warn: {task_warns}"
        )

    print(f"\n{'=' * 80}")
    print(
        f"TOTAL: {total_tests} tests | ✓ Pass: {passed_tests} ({passed_tests / total_tests * 100:.1f}%) | "
    ...
    total_effort_hours = sum(float(t["effort"].split("-")[0].split()[0]) for t in phase2_tasks)
