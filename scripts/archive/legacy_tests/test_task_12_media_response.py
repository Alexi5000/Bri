...
async def test_agent_media_response():
    ...
    try:
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            try:
                # Process query
                response = await agent.chat(message=query, video_id=video_id)
                ...
                if response.frame_contexts:
                    ...
                    for j, ctx in enumerate(response.frame_contexts[:3], 1):
                        print(
                            f"       {j}. [{MediaUtils.format_timestamp(ctx.timestamp)}] {ctx.description[:50]}..."
                        )
...
def test_frame_context_structure():
    ...
    print(
        f"   - Timestamp: {frame_ctx.timestamp}s ({MediaUtils.format_timestamp(frame_ctx.timestamp)})"
    )
