...
def test_suggestion_generation():
    ...
    class MockAgent:
        ...
        def _generate_suggestions(self, user_message, response, video_id):
            ...
            agent._suggest_visual_followups = GroqAgent._suggest_visual_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_audio_followups = GroqAgent._suggest_audio_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_object_followups = GroqAgent._suggest_object_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_timestamp_followups = GroqAgent._suggest_timestamp_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_summary_followups = GroqAgent._suggest_summary_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_general_followups = GroqAgent._suggest_general_followups.__get__(
                agent, GroqAgent
            )
            agent._suggest_exploration_followups = GroqAgent._suggest_exploration_followups.__get__(
                agent, GroqAgent
            )
            agent._detect_additional_content = GroqAgent._detect_additional_content.__get__(
                agent, GroqAgent
            )
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        suggestions = mock_agent._generate_suggestions(
            test_case["user_message"], test_case["response"], "test_video_id"
        )
        ...
        if "expected_type" in test_case:
            ...
            if query_type == test_case["expected_type"]:
                ...
                print(
                    f"⚠ Query classified as '{query_type}' (expected '{test_case['expected_type']}')"
                )
