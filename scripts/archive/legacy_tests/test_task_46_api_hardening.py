...
from mcp_server.versioning import parse_version, APIVersion, get_version_info
...
def test_request_validation():
    ...
    try:
        req = ValidatedToolExecutionRequest(video_id="test_video_123", parameters={"interval": 2.0})
    ...
    try:
        req = ValidatedToolExecutionRequest(video_id="../../../etc/passwd", parameters={})
    ...
    try:
        req = ValidatedProcessVideoRequest(tools=["extract_frames", "caption_frames"])
    ...
    try:
        req = ValidatedProcessVideoRequest(tools=["tool" + str(i) for i in range(15)])
    ...
    try:
        req = ValidatedProgressiveProcessRequest(video_path="data/videos/test.mp4")
    ...
    try:
        req = ValidatedProgressiveProcessRequest(video_path="data/videos/test.txt")
...
def test_response_models():
    ...
    response = create_standard_response(
        data={"result": "success", "count": 10}, execution_time=0.123
    )
    ...
    error = ErrorDetail(
        code="VIDEO_NOT_FOUND", message="Video not found", suggestion="Check video ID and try again"
    )
    response = create_standard_response(error=error, execution_time=0.050)
    ...
    response = create_paginated_response(
        items=items[:5], page=1, page_size=5, total_items=10, execution_time=0.075
    )
    assert len(response.data) == 5
    assert response.pagination["total_pages"] == 2
    assert response.pagination["has_next"] is True
    print(
        f"✓ Paginated response: page {response.pagination['page']}/{response.pagination['total_pages']}"
    )
...
def test_circuit_breaker():
    ...
    breaker = CircuitBreaker(name="test_service", failure_threshold=3, recovery_timeout=5.0)
...
def test_api_versioning():
    ...
    print(
        f"✓ Version info: current={info['current_version']}, supported={info['supported_versions']}"
    )
