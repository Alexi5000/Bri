...
def setup_test_data(db: Database, video_id: str) -> None:
    ...
    metadata = VideoMetadata(
        duration=60.0, fps=30.0, width=1920, height=1080, codec="h264", file_size=10485760
    )
    db.execute_update(
        """
    ...
    for frame in frames_data:
        db.execute_update(
            ...
            """,
            (
                f"ctx_{uuid.uuid4().hex[:16]}",
                video_id,
                "frame",
                frame.timestamp,
                json.dumps(frame.model_dump()),
            ),
        )
    ...
    transcript = Transcript(
        segments=[
            TranscriptSegment(
                start=3.0, end=7.0, text="Look at that beautiful park", confidence=0.94
            ),
            TranscriptSegment(
                start=8.0, end=12.0, text="The dog is so happy running around", confidence=0.96
            ),
            TranscriptSegment(
                start=14.0, end=18.0, text="Let's play fetch with the ball", confidence=0.93
            ),
    ...
    for detection in detections_data:
        db.execute_update(
            ...
            (
                ...
                json.dumps(detection.model_dump()),
            ),
        )
...
def test_get_context_at_timestamp(builder: ContextBuilder, video_id: str) -> None:
    ...
    print(f"Captions: {len(context.captions)}")
    print(
        f"Transcript segment: {context.transcript_segment.text if context.transcript_segment else 'None'}"
    )
