...
def test_error_handler_integration():
    ...
    message = ErrorHandler.handle_tool_error(
        "frame_extractor", error, available_tools=["audio_transcriber"]
    )
    ...
    suggestion = ErrorHandler.suggest_fallback("What did they say?", ["captions"], ["transcripts"])
    ...
    message = ErrorHandler.handle_processing_error(
        "video123", error, completed_steps=["extraction"]
    )
    ...
    message = ErrorHandler.format_error_for_user(error, context={"query": "Test query"})
    assert len(message) > 0
...
def test_agent_error_handling_simulation():
    ...
    error_msg = ErrorHandler.handle_tool_error(
        "caption_frames", error, available_tools=["transcribe_audio"]
    )
