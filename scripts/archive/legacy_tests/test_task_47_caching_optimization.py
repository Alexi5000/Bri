    ...
    try:
        ...
        async def test_lazy_load():
            result = await prefetcher.lazy_load_paginated(
                "test_video", "frames", page_size=10, page=0
            )
...
def test_json_compression():
    ...
    test_data = {
        "video_id": "test_123",
        "frames": [{"timestamp": i, "data": f"frame_{i}" * 10} for i in range(100)],
    }
...
def test_image_compression():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(
                f"✓ Image compression: {original_size} → {compressed_size} bytes ({ratio:.1f}% reduction)"
            )
...
def test_response_compression():
    ...
    test_response = json.dumps(
        {
            "status": "success",
            "data": {"results": [{"id": i, "value": f"data_{i}" * 20} for i in range(100)]},
        }
    )
    ...
    print(
        f"✓ Response compression: {original_size} → {compressed_size} bytes ({ratio:.1f}% reduction)"
    )
    assert compressed_size < original_size, "Compressed should be smaller"
