...
def main():
    ...
    parser.add_argument(
        "backup_path", type=str, nargs="?", help="Path to backup file (default: latest backup)"
    )
    parser.add_argument(
        "--no-verify", action="store_true", help="Skip backup verification before restore"
    )
    ...
    try:
        ...
        if args.list:
            ...
            for i, backup in enumerate(backups, 1):
                ...
                if "tables" in backup:
                    print(
                        f"   Tables: {', '.join(f'{k}({v})' for k, v in backup['tables'].items())}"
                    )
                print()
            return 0
